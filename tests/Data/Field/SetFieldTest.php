...
/**
 * Methods for testing SetField.
 */
final class SetFieldTest extends TestCase
{

    /**
     * Test methods specific for SetField.
     *
     * @return void
     */
    public function testIfWorks() : void
    {
        ...
        $this->assertEquals($dicts['main'][0] . ',' . $dicts['main'][1], $field->getDictValue('one,two'));
        $this->assertEquals($dicts['main'][0], $field->getDictValue('one', 'main'));
        $this->assertEquals($dicts['main'][0] . ',' . $dicts['main'][1], $field->getDictValue('one,two', 'main'));
        $this->assertEquals($dicts['pl:pl'][0], $field->getDictValue('one', 'pl:pl'));
        $this->assertEquals($dicts['pl:pl'][0] . ',' . $dicts['pl:pl'][1], $field->getDictValue('one,two', 'pl:pl'));
